...
namespace Kyber {
    ...
    struct RenderEvent final : public Event
    {
        u32 x;
        u32 y;
        u32 w;
        ...
        constexpr explicit RenderEvent(u32 x, u32 y, u32 w, u32 h) noexcept
            : x(x), y(y), w(w), h(h)
        {
        }

        virtual constexpr i32 GetEventCategory() const override { return EventCategory::EventCategoryRender; }

        inline static constexpr EventType GetStaticType() { return EventType::Render; }
        virtual constexpr EventType GetType() const override { return GetStaticType(); }
    };
